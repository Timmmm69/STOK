...
Тесты не зависят от пользовательских данных разработки, порядка выполнения или производственных секретов. Playwright создаёт тестовую подписанную сессию через официальный тестовый модуль Better Auth; этот модуль не подключён к рабочему приложению и не создаёт скрытого способа входа. Реальный Google-вход отдельно проверяется вручную после создания ключей Google Cloud. Факт запуска и результат существенной задачи записываются в execution plan.
