...
- `pnpm db:seed` — идемпотентно записать технический маркер и приглашённого владельца из `AUTH_ALLOWED_EMAIL`.
...
Техническая таблица `_harness_state` доказывает migration/seed и не принимает бизнес-поля. Seed не создаёт организацию или магазин: это делает только защищённый пользовательский сценарий после входа.
